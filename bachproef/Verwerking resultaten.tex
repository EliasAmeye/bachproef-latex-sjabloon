...
Om deze resultaten met elkaar te vergelijken, wordt er gebruik gemaakt van niet-gepaarde t-testen. Hierin stellen we 𝜇\index{1} gelijk aan het populatiegemiddelde van de scores van Patroni en 𝜇\index{2} aan het populatiegemiddelde van PostgreSQL Automatic Failover (PAF). Op deze manier willen we aantonen dat Patroni beter scoort dan PostgreSQL Automatic Failover (PAF). We nemen als $\alpha = 5\%$. In Figuur~\ref{fig:R berekening Patroni en PostgreSQL Automatic Failover (PAF)} wordt deze hypothese berekend in R.
...
Wanneer we kijken naar Patroni en Replication Manager (repmgr), dan kunnen we opnieuw een niet-gepaarde t-test opstellen. Hierin stellen we 𝜇\index{1} gelijk aan het populatiegemiddelde van de scores van Patroni en 𝜇\index{2} aan het populatiegemiddelde van Replication Manager (repmgr). Op deze manier willen we aantonen dat Patroni beter scoort dan Replication Manager (repmgr). We nemen als $\alpha = 5\%$. In Figuur~\ref{fig:R berekening Patroni en Replication Manager (repmgr)} wordt deze hypothese berekend in R.
...
Kijken we naar PostgreSQL Automatic Failover (PAF) en Replication Manager (repmgr), dan kunnen we opnieuw een niet-gepaarde t-test opstellen. Hierin stellen we 𝜇\index{1} gelijk aan het populatiegemiddelde van de scores van PostgreSQL Automatic Failover (PAF) en 𝜇\index{2} aan het populatiegemiddelde van Replication Manager (repmgr). Op deze manier willen we aantonen dat PostgreSQL Automatic Failover (PAF) beter scoort dan Replication Manager (repmgr). We nemen als $\alpha = 5\%$. In Figuur~\ref{fig:R berekening PostgreSQL Automatic Failover (PAF) en Replication Manager (repmgr)} wordt deze hypothese berekend in R.
