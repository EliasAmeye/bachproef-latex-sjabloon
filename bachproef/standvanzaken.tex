...
%Hiermee zal worden gekeken naar de nadruk die gelegd wordt bij elke oplossing. Op deze manier kan gekeken worden naar welke requirements regelmatig terugkomen, en dus welke kunnen gebruikt worden om later in het onderzoek verschillende oplossingen met elkaar te vergelijken. Aan de hand van deze requirements kan geschift worden tussen de bestaande High Availability PostgreSQL cluster oplossingen om hier dan twee oplossingen uit te halen. 
...
\section{\IfLanguageName{dutch}{Databank}{Databank}}
\label{sec:Databank}
...
\section{\IfLanguageName{dutch}{PostgreSQL}{PostgreSQL}}
...
Volgens DB-Engines is PostgreSQL de vierde database die vandaag de dag het meest gebruikt wordt en de tweede meest gebruikte open source database, na MySQL~\autocite{DBEngines2021}. DB-Engines verklaarde PostgreSQL in 2017, 2018 en 2020 het DBMS (Database management system) van het jaar~\autocite{DBEngines2021a}.
...
\subsection{\IfLanguageName{dutch}{Standby mogelijkheden}{Standby mogelijkheden}}
...
\subsection{\IfLanguageName{dutch}{Replication Manager (repmgr)}{Replication Manager (repmgr)}}
...
%\subsubsection{\IfLanguageName{dutch}{De implementatie als een PostgreSQL extentie}{De implementatie als een PostgreSQL extentie}}
%\label{subsubsec:De implementatie als een PostgreSQL extentie}
1. De implementatie als een PostgreSQL extentie.


%\subsubsection{\IfLanguageName{dutch}{Replicatie cluster monitoring}{Replicatie cluster monitoring}}
%\label{subsubsec:Replicatie cluster monitoring}
2. Replicatie cluster monitoring.


%\subsubsection{\IfLanguageName{dutch}{Standby klonen aan de hand van pg\_basebackup of Barman}{Standby klonen aan de hand van pg\_basebackup of Barman}}
...
\begin{itemize}
    ...
    De back-up wordt gemaakt over een gewone PostgreSQL verbinding maakt gebruik van het replicatieprotocol. De server moet ook worden geconfigureerd om ten minste één sessie beschikbaar te laten voor de back-up~\autocite{PostgreSQL2021c}. %(https://www.postgresql.org/docs/10/app-pgbasebackup.html)
    \newline
    \item Barman: Barman of pgbarman staat voor Backup en Recovery Manager. Het is een open source beheertool voor disaster recovery van PostgreSQL servers. Het is geschreven in Python. Barman laat toe om op afstand van meerdere servers in bedrijfskritische omgevingen back-ups uit te voeren. Het helpt ook database beheerders tijdens een herstelfase~\autocite{Barman2020}~\autocite{Barman2020a}. %(https://www.pgbarman.org/about/)(https://www.pgbarman.org/)
\end{itemize}


%\subsubsection{\IfLanguageName{dutch}{Promotie standby server}{Promotie standby server}}
...
%\subsubsection{\IfLanguageName{dutch}{Cascading Standby Support}{Cascading Standby Support}}
...
%\subsubsection{\IfLanguageName{dutch}{Vereenvoudigen van het beheer van WAL-retentie en ondersteuning voor replicatiesleuven}{Vereenvoudigen van het beheer van WAL-retentie en ondersteuning voor replicatiesleuven}}
...
%\subsubsection{\IfLanguageName{dutch}{Switchover ondersteuning voor rolswitching tussen primary en standby}{Switchover ondersteuning voor rolswitching tussen primary en standby}}
